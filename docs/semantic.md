...
- **D3 触发源**：`src/index.ts` mtime `2026-09-19 21:51:42` 晚于本文件 `2026-09-14 10:14:15`——但那是**工作区未提交**改动的时间，不是提交时间。
- **取证（可复现）**：`git log --since='2026-09-14 10:35' --oneline -- src/index.ts src/client/index.ts` → **空**（09-14 之后无任何 impl 提交）；`git diff -- src/index.ts` → 仅 1 处改动（+2/−1）。
- **改动是什么**：`JsonValue` 类型导入从 `@deepseek-ai/dsh-tools` 迁到 `@deepseek-ai/dsh-util-values`（`src/index.ts:15-16`，DSH 0.1.5/0.1.6 平台适配；type-only，无运行时语义——同一波迁移见 `dsh-agent-teams/src/tools.ts:15` 同款注释）。
- **结论：假报**——该改动属**平台升级波次**，不属于本条语义（浏览器控制台感知）。告警来自 impl 清单的**文件粒度**：`src/index.ts` 被无关语义的改动碰到，mtime 即前进。本文档的契约（§4）、不变量（§3）、失败面（§5）、验收表（§7）**无需修订**。
- **该改动已构建**：`lib/index.js` mtime `2026-09-20 12:07:07` 晚于 src 改动、早于 web 进程启动 `2026-09-22 12:29:33` ⇒ 线上加载的就是该版本。
- **附带事实（未回写，待落盘后一并处理）**：该未提交改动在文件前部插入 1 行，使 §4.5 引用的行号整体 +1（service `:87→:88`、`apply` `:115→:116`、工具面 `:119/:141→:121/:143`、路由 `:157→:158`）。本文件行号仍锚定**已提交**版本（HEAD `3d9518a`）——**该改动落盘后需重新核对行号**。
